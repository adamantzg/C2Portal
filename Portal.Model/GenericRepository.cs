...
namespace Portal.Model
{
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : class
    {
            ...
            string includeProperties = "")
        {
            ...
            //        return orderBy(query).Skip(skip.Value).ToList();
            //    }                    
            //    else {
            //        if (skip == null)
            //            return orderBy(query).Take(take.Value).ToList();
            //        return orderBy(query).Take(take.Value).Skip(skip.Value).ToList();
            //    }                
            //}
            //else {
            //    return query.ToList();
        ...
        //public virtual void LoadCollection(TEntity entity, Expression<Func<TEntity, ICollection<TEntity>>> collExpression)
        //{
        //    context.Entry(entity).Collection(collExpression).Load();
        //}
